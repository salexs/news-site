...
export class NewsService {
    ...
    getData(currentUser: string, page:number=1,search:string,filter:string): Observable<any> {
        ...
        if (currentUser == undefined) {
            return this.http.get('http://localhost:8000/api/news/?page='+page+(search==undefined?'':'&search='+search)+(filter==undefined?'':'&filter='+filter), options)
